...
class IncomeView extends StatefulWidget {
  ...
  IncomeView({Key? key, required this.user}) : super(key: key);
...
class _IncomeViewState extends State<IncomeView> {
  TextEditingController incomeDetailCtrl = TextEditingController(text: '');
  TextEditingController incomeAmountCtrl = TextEditingController(text: '');
  TextEditingController dateIncomeCtrl = TextEditingController(text: '');
  ...
  Future<void> _openCalendar() async {
    ...
    if (_picker != null) {
      setState(() {
        ...
        dateIncomeCtrl.text = convertToThaiDate(_picker);
      });
    }
  }
                    ...
                    child: Column(
                      children: [
                        ...
                        ElevatedButton(
                          onPressed: () {
                            //Validate
                            if (incomeDetailCtrl.text.trim().length == 0) {
                              showWaringDialog(
                                  context, 'ป้อนรายละเอียดเงินเข้าด้วย....');
                            } else if (incomeAmountCtrl.text.trim().length ==
                                0) {
                              showWaringDialog(
                                  context, 'ป้อนจำนวนเงินด้วย....');
                            } else if (dateIncomeCtrl.text.trim().length == 0) {
                              showWaringDialog(
                                  context, 'เลือกวันที่เงินเข้าด้วย....');
                            } else {
                              //ส่งข้อมูลที่ผู้ใช้ป้อน/เลือกไปให้ API เพื่อบันทึกการกินลงฐานข้อมูล
                              //แพ็กข้อมูลที่จะส่งรวมกัน
                              Money money = Money(
                                moneyDetail: incomeDetailCtrl.text.trim(),
                                moneyInOut: incomeAmountCtrl.text.toString(),
                                moneyDate: dateIncomeCtrl.text.trim(),
                                moneyType: '1',
                                userId: widget.user
